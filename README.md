...
> A self-hosted, browser-based GUI for pushing, pulling, and managing GitHub repositories Ã¢ÂÂ built to replace the terminal `git add / commit / push` workflow with drag-and-drop file uploads, automatic `.gitignore` handling, and a proper account manager for multiple GitHub identities.
...
- Optionally, **generate commit messages and READMEs with AI** (see [AI Tools](#ai-tools--requires-setup) below Ã¢ÂÂ this one needs a bit of configuration)

Everything runs through your own Next.js server talking directly to `api.github.com` Ã¢ÂÂ there's no third-party backend in between.
...
- **Drag-and-drop** whole folders (recursively) or individual files directly onto the push dialog Ã¢ÂÂ no reliance on the OS's native file picker, which can be unreliable on some Linux/browser combinations
...
- **`.git/` is always excluded**, unconditionally and non-overridably Ã¢ÂÂ GitHub's API rejects any path containing a `.git` component outright, so this is enforced regardless of your `.gitignore` settings, both client- and server-side
...
- **Live progress feedback** at every stage Ã¢ÂÂ folder scanning, file caching/encoding, and the actual GitHub upload each show their own progress, pinned to a fixed spot in the dialog so it's never hidden by scrolling
...
- No artificial cap on file count Ã¢ÂÂ GitHub's own Trees API doesn't have one either; only total payload size is guarded against
...
- The `/api/projects/*` and `/api/changelog/*` routes Ã¢ÂÂ these reference a `Project`/`Changelog` Prisma model that no longer exists in `prisma/schema.prisma` (only `User` and `Account` are defined)

None of this is reachable from the real app (`src/app/page.tsx` only renders `DashboardView` from `src/components/dashboard/`, `AccountReposView`, `RepoDetailView`, `FileEditor`, and `AIToolsView`), so it doesn't affect normal use Ã¢ÂÂ but if you're exploring the codebase, don't be surprised to find it. Safe to delete if you want a cleaner tree.
...
Both are required. `DATABASE_URL` **must be a relative path** (`file:./db/custom.db`) Ã¢ÂÂ an absolute path baked in from a different machine/environment will fail with a permissions or "unable to open database file" error.
...
Sign up on first load (this creates a local `User` row), then add a GitHub account from within the app using a **Personal Access Token** (classic or fine-grained, with `repo` scope). That token is what powers push/pull/branch/commit operations Ã¢ÂÂ it's stored per-account in the local database.
...
| `npm run db:reset` | Reset the database (Ã¢ÂÂ Ã¯Â¸Â destructive) |
...
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ api/              # Route handlers: auth, accounts, github/*, ai/*
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ layout.tsx
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ page.tsx           # Top-level view router
Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ components/
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ auth/               # Login/register
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ accounts/            # Add/manage GitHub accounts
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ dashboard/            # Account list (the real dashboard)
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ repos/                 # Repo list, repo detail (Files/Branches/Commits), New Repo dialog
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ files/                  # File browser, editor, Push/Pull dialogs (the core feature)
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ branches/                # Branch management
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ commits/                   # Commit history
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ ai-tools/                   # AI commit message / README generator UI
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ layout/                      # App shell, sidebar, top bar
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ ui/                           # shadcn/ui primitives
Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ lib/
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ github.ts            # All GitHub REST API calls (blobs, trees, commits, refs, archives)
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ gitignore.ts          # .gitignore pattern matching/filtering
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ auth.ts                 # JWT session handling
Ã¢ÂÂ   Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ db.ts                    # Prisma client
Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ services/api.ts        # Client-side fetch wrappers for the API routes above
Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂ store/appStore.ts     # Zustand global state (view routing, accounts, selected repo, etc.)
```
