...
// -------- Repositories --------
export async function listRepos(token: string, username: string, page = 1, perPage = 30) {
  const url = username === '*' 
...
export async function searchRepos(token: string, query: string, page = 1, perPage = 30) {
...
// -------- Contents --------
export async function getContents(token: string, owner: string, repo: string, path: string, ref?: string) {
  const params = new URLSearchParams({ path });
  if (ref) params.set('ref', ref);
  return ghFetch<import('@/types').GitHubContent[]>(
    `${GITHUB_API}/repos/${owner}/${repo}/contents?${params.toString()}`,
    token,
  );
}
...
export async function createOrUpdateFile(
  ...
  repo: string,
  path: string,
  content: string,       // plain-text — we base64-encode here
  message: string,
...
// -------- Branches --------
export async function listBranches(token: string, owner: string, repo: string, page = 1, perPage = 100) {
...
export async function createBranch(token: string, owner: string, repo: string, branch: string, fromSha: string) {
...
export async function getBranchSha(token: string, owner: string, repo: string, branch: string) {
...
// -------- Commits --------
export async function listCommits(token: string, owner: string, repo: string, sha?: string, page = 1, perPage = 30) {
